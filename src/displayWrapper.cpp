...
const char HelloWorld[] = "Ich liebe dich!";
...
void displayPrintTextFast(const char* text, uint8_t rotation)
{
...
void displayTestText(const char* text) {
...
void displayDay(uint8_t day)
{
    ...
    display.setPartialWindow(0, 0, display.width()/3, display.height());
    display.setTextColor(GxEPD_BLACK);
    ...
    display.getTextBounds(text, 0, 0, &tbx, &tby, &tbw, &tbh);
    // center bounding box by transposition of origin:
    uint16_t x = ((display.width()/3 - tbw) / 2) - tbx;
    uint16_t y = ((display.height() - tbh) / 2) - tby;
    //uint16_t x = 5;
    //uint16_t y = 70;
    ...
    do
    {
      //display.writeFillRect(0, 0, display.width()/3, display.height(), GxEPD_WHITE);
      //display.fillScreen(GxEPD_WHITE);
      display.setCursor(x, y);
      display.print(text);
    }
...
#define MAX_STRING_LENTGH 100

void displayText(const char* pText, uint8_t line)
{
    display.setPartialWindow(display.width()/3, 0, display.width()*2/3, display.height());
    display.setTextColor(GxEPD_BLACK);
    ...
    int16_t tbx, tby; uint16_t tbw, tbh;
    display.getTextBounds(pText, 0, 0, &tbx, &tby, &tbw, &tbh); // center bounding box by transposition of origin:

    uint16_t x = ((display.width()*2/3 - tbw) / 2) - tbx;
    uint16_t y = ((display.height() - tbh) / 2) - tby;
    ...
    do
    {
      if(line == 1) display.fillScreen(GxEPD_WHITE);
      display.setCursor(x, y);
      display.print(pText);
    }
    while (display.nextPage());
